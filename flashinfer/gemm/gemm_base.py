...
from .gemm_bias import (
    FP4Type,
    _is_fp8,
    _GEMM_BIAS_TUNING_CONFIG,
    _cudnn_gemm_bias_runner,
    _cudnn_gemm_bias_requirement,
    _check_gemm_bias_problem_size,
    _heuristic_func_gemm_bias,
    build_cudnn_gemm_bias_graph,
    build_cudnn_gemm_fp8_bias_graph,
    build_cudnn_gemm_fp4_bias_graph,
)
...
# ---------------------------------------------------------------------------
# gemm_bias public API
# ---------------------------------------------------------------------------


@backend_requirement(
    backend_checks={
        "cudnn": _cudnn_gemm_bias_requirement,
    },
    common_check=_check_gemm_bias_problem_size,
    heuristic_func=_heuristic_func_gemm_bias,
)
@flashinfer_api(trace=gemm_bias_trace)
def gemm_bias(
    a: torch.Tensor,
    b: torch.Tensor,
    bias: torch.Tensor,
    ...
    fused operation using the cuDNN backend.

    Supported input types:

    - **Dense** (``bfloat16``, ``float16``, ``float32``): SM80+
    - **FP8** (``float8_e4m3fn``, ``float8_e5m2``): SM89+, requires per-tensor
      ``a_scale`` and ``b_scale``
    - **NVFP4** (``uint8`` packed): SM100+, requires ``fp4_dtype=FP4Type.NVFP4``
      and per-block ``a_descale``/``b_descale`` in ``float8_e4m3fn``
    - **MXFP4** (``uint8`` packed): SM100+, requires ``fp4_dtype=FP4Type.MXFP4``
      and per-block ``a_descale``/``b_descale`` in ``uint8`` (fp8_e8m0)

    Parameters
    ----------
    a: torch.Tensor
        Input matrix, shape ``(m, k)`` or ``(batch, m, k)``.
        For FP4: ``uint8`` packed (logical shape ``(m, k)`` stored as ``(m, k//2)``).

    b: torch.Tensor
        Weight matrix in **column-major** layout, shape ``(k, n)`` or
        ``(batch, k, n)``. For FP4: ``uint8`` packed (stored as ``(k//2, n)``).

    bias: torch.Tensor
        Bias vector, shape ``(n,)`` for a global bias or ``(batch, 1, n)``
        for a per-batch bias. Dtype must match ``out_dtype``.

    a_scale: Optional[torch.Tensor]
        Per-tensor dequantization scale for ``a``. Required for FP8; ``None``
        for all other input types.

    b_scale: Optional[torch.Tensor]
        Per-tensor dequantization scale for ``b``. Required for FP8; ``None``
        for all other input types.

    a_descale: Optional[torch.Tensor]
        Per-block dequantization scale tensor for ``a`` (FP4 inputs only).
        Shape: ``(m_pad, k_pad // block_size)``. Dtype: ``float8_e4m3fn``
        for NVFP4 or ``uint8`` (fp8_e8m0) for MXFP4.

    b_descale: Optional[torch.Tensor]
        Per-block dequantization scale tensor for ``b`` (FP4 inputs only).
        Shape: ``(k_pad // block_size, n_pad)``. Same dtype as ``a_descale``.

    alpha: Optional[torch.Tensor]
        Global scale applied to the GEMM result before bias addition:
        ``out = (a @ b) * alpha + bias``. Shape ``(1,)`` float32. FP4 only.

    fp4_dtype: FP4Type
        FP4 quantization variant. ``FP4Type.NVFP4`` (block_size=16, descale
        dtype ``float8_e4m3fn``) or ``FP4Type.MXFP4`` (block_size=32, descale
        dtype ``uint8`` fp8_e8m0). Defaults to ``FP4Type.NVFP4``; only
        meaningful when ``a_descale`` is provided.

    out_dtype: Optional[torch.dtype]
        Output dtype. Defaults to ``a.dtype`` for dense inputs, ``bfloat16``
        for FP8/FP4. Supported: ``bfloat16``, ``float16``, ``float32``.
    ...
    >>> out = flashinfer.gemm_bias(a, b, bias)
    >>> out.shape
    torch.Size([48, 128])
    >>> out.dtype
    torch.bfloat16
    ...
    if out is None:
        # For FP4: b has packed shape [k//2, n], so b.shape[-1] = N directly.
        expected_shape = (*a.shape[:-1], b.shape[-1])
        out = torch.empty(expected_shape, device=a.device, dtype=resolved_out_dtype)

    workspace_buffer = _get_cache_buf("gemm_bias_workspace", DEFAULT_WORKSPACE_SIZE, a.device)

    if backend == "auto":
        backends = gemm_bias.suitable_auto_backends
    else:
        backends = [backend]

    tuner = AutoTuner.get()
    runners = [_cudnn_gemm_bias_runner() for _ in backends]

    inputs = [a, b, bias, a_scale, b_scale, a_descale, b_descale, alpha, fp4_dtype, out, workspace_buffer]
    runner, tactic = tuner.choose_one(
        "gemm_bias",
        runners,
        _GEMM_BIAS_TUNING_CONFIG,
